document.addEventListener('DOMContentLoaded', () => {
    // שימוש ב-querySelector לבחירת אלמנטים מה-DOM
    const container = document.querySelector('#container');
    const numButtons = 10;
    const numDifferentColors = 5;
    let gameActive = true;
    let timerInterval;
    ...
    function checkLevelUp() {
        if (wins > 2) {
            showMessage('עבר לשלב 2!').then(() => {
                window.location.href = "../html/game1.html"; // שנה את הקישור לדף המתאים לשלב 2
            });
        }
    }
